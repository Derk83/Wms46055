...
@pytest.mark.django_db
def test_weekly_report_includes_range(client, manager_groups):
    ...
    item = InventoryItem.objects.create(
        part_number="RPT-W1", name="In range item", quantity_on_hand=10
    )
    requester = User.objects.create_user(username="req-w", password="pw")
    mr_in = create_material_request(
        creator=requester,
        requestor_name="In range",
        building_room="Rm",
        location="Loc",
        notes="",
        lines=[{"item": item, "quantity": 1, "notes": ""}],
    )
    mr_out = create_material_request(
        creator=requester,
        requestor_name="Out of range",
        building_room="Rm",
        location="Loc",
        notes="",
        lines=[{"item": item, "quantity": 1, "notes": ""}],
    )
    MaterialRequest.objects.filter(pk=mr_in.pk).update(created_at=two_days_ago)
    MaterialRequest.objects.filter(pk=mr_out.pk).update(created_at=six_days_ago)

    # Range = last 5 days (so 2-days-ago is in, 6-days-ago is out)
    start = (today - timedelta(days=5)).date().isoformat()
    end = today.date().isoformat()
    response = client.get(
        reverse("reports_weekly"), {"start": start, "end": end}
    )
    body = response.content.decode("utf-8")
    new_section = body.split("New material requests")[1].split("Delivery confirmations")[0]
    assert mr_in.request_number in new_section
    assert mr_out.request_number not in new_section
