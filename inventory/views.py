...
def _build_report_context(rng, *, kind, request):
    """Aggregate everything the daily report template needs.

    The weekly report is auto-generated by the `generate_weekly_report`
    management command and renders directly from its own context (see
    inventory/management/commands/generate_weekly_report.py), so this
    helper is now daily-only.
    """
...
def reports_pdf(request, kind):
    ...
    The weekly report is no longer served via this route; it's auto-generated
    every Friday morning by the `generate_weekly_report` management command.
    """
    from inventory.reports import parse_date_range, preset_dates
    ...
    ctx = _build_report_context(rng, kind="daily", request=request)
    template_name = "inventory/reports_pdf.html"
    from django.conf import settings as django_settings
    from django.template.loader import render_to_string
    from weasyprint import HTML

    logo_path = django_settings.BASE_DIR / "inventory" / "static" / "inventory" / "img" / "blackbox-logo.png"
    ctx["logo_uri"] = logo_path.as_uri()

    html = render_to_string(template_name, ctx, request=request)
    pdf = HTML(string=html, base_url=str(django_settings.BASE_DIR)).write_pdf()
    response = HttpResponse(pdf, content_type="application/pdf")
    filename_date = rng.start.date().isoformat()
    response["Content-Disposition"] = (
